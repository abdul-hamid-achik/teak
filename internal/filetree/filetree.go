...
type Model struct {
	...
	entriesGeneration uint64 // changes whenever the persistent entry root changes
...
func (m *Model) RefreshDir(dir string) {
	...
	if updated, found := refreshEntriesInSlice(m.Entries, m.Root, dir, m.gitignorePatterns); found {
		m.Entries = updated
		m.entriesGeneration++
	}
	m.invalidateFlatCache()
...
// SnapshotForRefresh captures the persistent entry root in constant time.
// Interactive entry mutations use copy-on-write, so the captured slices remain
// immutable while a background refresh reads them.
func (m Model) SnapshotForRefresh() Model {
	m.cachedFlat = nil
	m.sharedFlatCache = nil
	m.filterCancel = nil
	return m
}
...
func (m Model) Refresh(ctx context.Context, dir string) (RefreshResult, error) {
	...
	if filepath.Clean(dir) == filepath.Clean(m.Root) {
		...
		return m.prepareRefreshResult(ctx, entries, patterns)
	}
	entries, _, err := refreshEntriesInSliceContext(ctx, m.Entries, m.Root, filepath.Clean(dir), patterns, budget)
	if err != nil {
		return RefreshResult{}, err
	}
	return m.prepareRefreshResult(ctx, entries, patterns)
...
// ApplyRefresh installs a tree and projection prepared by Refresh. It returns
// false when entry or view state changed while the command was running; callers
// can then retry from the latest persistent root without merging in Update.
func (m *Model) ApplyRefresh(result RefreshResult) bool {
	if !result.prepared || result.baseGeneration != m.entriesGeneration ||
		result.showHidden != m.showHidden || result.showGitIgnored != m.showGitIgnored || result.filter != m.filter {
		return false
	}
	selectedPath := m.selectedPathCached()
	m.cancelFilter()
	m.filterGeneration++
	m.pendingSelection = ""
	m.Entries = result.Entries
	m.gitignorePatterns = result.GitignorePatterns
	m.entriesGeneration++
	m.cachedFlat = result.projection
	...
	m.ensureCursorVisibleForLength(len(result.projection))
	return true
}

func (m Model) prepareRefreshResult(ctx context.Context, entries []Entry, patterns []string) (RefreshResult, error) {
	var source []Entry
	visited := 0
	if err := flattenVisibleEntriesContext(ctx, entries, 0, m.showHidden, m.showGitIgnored, &visited, &source); err != nil {
		return RefreshResult{}, err
	}
	if source == nil {
		source = make([]Entry, 0)
	...
	indexByPath := make(map[string]int, len(projection))
	...
	return RefreshResult{
		...
		baseGeneration:    m.entriesGeneration,
...
func flattenVisibleEntriesContext(ctx context.Context, entries []Entry, depth int, showHidden, showGitIgnored bool, visited *int, flat *[]Entry) error {
	for _, entry := range entries {
		...
		*flat = append(*flat, entry)
		if entry.IsDir && entry.Expanded && entry.Children != nil {
			if err := flattenVisibleEntriesContext(ctx, entry.Children, depth+1, showHidden, showGitIgnored, visited, flat); err != nil {
				return err
			}
		}
	}
...
func (m *Model) selectedPathCached() string {
...
func refreshEntriesInSliceContext(ctx context.Context, entries []Entry, rootDir, dir string, gitignorePatterns []string, budget *refreshBudget) ([]Entry, bool, error) {
	for i := range entries {
		if err := ctx.Err(); err != nil {
		...
		children, found, err := refreshEntriesInSliceContext(ctx, entries[i].Children, rootDir, dir, gitignorePatterns, budget)
		if err != nil {
			return nil, false, err
		}
		if found {
...
func refreshEntriesInSlice(entries []Entry, rootDir, dir string, gitignorePatterns []string) ([]Entry, bool) {
	for i := range entries {
		if entries[i].Path == dir && entries[i].IsDir {
			updated := slices.Clone(entries)
			updated[i].Children = refreshEntriesPreservingExpansion(
				rootDir,
				dir,
				entries[i].Depth+1,
				gitignorePatterns,
		...
		children, found := refreshEntriesInSlice(entries[i].Children, rootDir, dir, gitignorePatterns)
		if found {
			updated := slices.Clone(entries)
			updated[i].Children = children
			return updated, true
...
func toggleEntries(entries []Entry, rootDir, path string, gitignorePatterns []string) ([]Entry, tea.Cmd, bool) {
	for i := range entries {
		if entries[i].Path == path && entries[i].IsDir {
			updated := slices.Clone(entries)
			updated[i].Expanded = !entries[i].Expanded
			if updated[i].Expanded && updated[i].Children == nil && !updated[i].Loading {
				updated[i].Loading = true
				dirPath := updated[i].Path
				depth := updated[i].Depth + 1
				cmd := func() tea.Msg {
		...
		children, cmd, found := toggleEntries(entries[i].Children, rootDir, path, gitignorePatterns)
		if found {
			updated := slices.Clone(entries)
			updated[i].Children = children
			return updated, cmd, true
...
func applyDirectoryLoadResult(entries []Entry, path string, children []Entry, loadErr error) ([]Entry, bool) {
	for i := range entries {
		if entries[i].Path == path && entries[i].IsDir {
			updated := slices.Clone(entries)
			updated[i].Loading = false
		...
		updatedChildren, found := applyDirectoryLoadResult(entries[i].Children, path, children, loadErr)
		if found {
			updated := slices.Clone(entries)
			updated[i].Children = updatedChildren
			return updated, true
		}
	}
	return entries, false
...
func (m *Model) ensureCursorVisibleForLength(entryCount int) {
	...
	maxScroll := max(0, entryCount-max(1, bodyHeight))
	if m.ScrollY > maxScroll {
		m.ScrollY = maxScroll
	}
}
