...
**Map positions through document-rewriting edits before installing them.** Formatting, rename and code actions replace the whole rope via `Buffer.ReplaceRopeSnapshot`, whose contract requires the *caller* to supply a cursor that is already a valid byte-boundary position in the new rope — validating an arbitrary cursor there could mean scanning a pathological giant line on the update goroutine. Production rewrite paths therefore map the cursor and selections through the edits first (`mapFormattingSelections`/`mapPositionThroughTextEdits`, `result.OffsetToPosition(mappedCursor)`) and only then install the snapshot. A caller that skips the mapping breaks silently: `ReplaceRopeSnapshot` cannot tell a stale position from a valid one. `Buffer.ClampCursor()` confines the cursor *and* every selection, but it has no production callers — it serves tests and ad-hoc repair, not any rewrite path.
